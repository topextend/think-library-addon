...
class ListenQueue extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        while (true) {
            ...
            $this->app->db->name('Queue')->where($where)->order('exec_time asc')->limit(100)->select()->each(function ($vo) use ($process, $output) {
                try {
    ...
    {
        return $this->app->db->name('Queue')->where(['code' => $code])->update($data);
    }
